...
IAM-002 adds `pnpm auth:verify`. Unit tests cover the offline whole-password policy, NFC, Argon2id, opaque token entropy, the dummy-hash path, account-state assurance, and fail-closed limiter behavior. The sequential integration suite uses real PostgreSQL/Redis plus the built Nest API to prove atomic registration rollback/contention, rotation, touch-versus-revoke safety, HMAC-only Redis keys, shared-network behavior, enumeration-safe responses, cookies/origin, sign-out, and raw-secret absence. `pnpm auth:calibrate` reports bounded representative Argon2id latency without changing policy under load.

IAM-005 adds `pnpm iam005:verify`. Unit tests cover the exact capability vocabulary, wildcard rejection, half-open assignment validity, and direct reuse of IAM-004 assurance evaluation. Its PostgreSQL integration suite proves immutable scope/template facts, fixed version authority, GIST overlap races, exact expiry/revocation, scope isolation, maker-checker and append-only evidence, temporary access, service-principal environment isolation, atomic privileged session containment, and revoke/use serialization. Protected role routes remain generic 404s.
...
UI-004 adds Docker-free application-shell tests. Route tests prove invisible route groups, collision-free internal destinations, exact mobile/account navigation, conservative production surface inputs, and multi-prefix current-location behavior. Component tests cover native GET search, supplied-only surface menus, desktop/compact navigation, safe loading/error recovery, keyboard interaction, and supported axe rules. `pnpm ui:shells:verify` runs deterministic policy validation, 15 injected negative fixtures, and focused unit/component suites; real-browser review remains authoritative for responsive geometry, zoom, forced colours, focus visibility, and assistive-technology output.
...
IAM-008's exact five-command family is `iam008:validate`, `iam008:self-test`, `iam008:test`, `iam008:integration-test`, and `iam008:verify`; there is no separate security command. The focused suites cover the typed registry, synthetic secret/PII sentinels, exact read projection, single-event recursion prevention, viewer accessibility/states, and real PostgreSQL migration, trigger, transaction, idempotency, concurrency, and Redis-independent durability guarantees. `iam008:verify` also composes IAM-006/IAM-007, database, outbox, and protected-route regressions. `/admin/audit` must remain fail closed, and no existing visual baseline may be accepted or altered to make this task green.
