...
if (process.argv.includes('--self-test')) {
  ...
  const cases = [
    ...
    ['missing API region', 'render.yaml', (value) => value.replace(/            region: frankfurt\r?\n            branch: main/, '            branch: main')],
    ['missing API health', 'render.yaml', (value) => value.replace(/            healthCheckPath: \/health\/ready\r?\n/, '')],
    ['public Worker', 'render.yaml', (value) => value.replace(/- type: worker\r?\n            name: noma-worker-staging/, '- type: web\n            name: noma-worker-staging')],
    ['missing Worker migration gate', 'render.yaml', (value) => value.replace(/            preDeployCommand: pnpm deploy:wait-for-migrations\r?\n/, '')],
    ...
    ['unsafe API filter', 'render.yaml', (value) => value.replace(/                - packages\/database\/\*\*\r?\n/, '')],
    ['unsafe Worker filter', 'render.yaml', (value) => value.replace(/                - apps\/worker\/\*\*\r?\n/, '')],
    ...
    ['database public ingress', 'render.yaml', (value) => value.replace(/            ipAllowList: \[\]\r?\n            storageAutoscalingEnabled/, '            ipAllowList:\n              - source: 0.0.0.0/0\n            storageAutoscalingEnabled')],
