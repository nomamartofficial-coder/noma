...
def validate(bundle=None):
    ...
    if len(tasks)!=153: raise ValidationError(f'expected 153 tasks, found {len(tasks)}')
    if sum(t['priority']=='P0' for t in tasks)!=142: raise ValidationError('expected 142 P0 tasks')
