...
| `identity_tokens` | Digest-only, purpose-bound one-time email verification and password recovery proofs with issue, expiry, consumption, invalidation, and replacement evidence. | Raw token never persists; same-purpose replacement and one-time consumption are authoritative and race-safe. |
