...
The API returns enumeration-safe outcomes, performs a dummy hash for unknown accounts, accepts ordinary password authentication only for the current authoritative `PENDING_EMAIL` and `ACTIVE` account states, and issues assurance `AUTHENTICATED` only. The same canonical eligibility rule governs sign-in and continuation of an existing session. Resolution rejects every ineligible User state before activity touch, while the conditional PostgreSQL touch rechecks User eligibility and security version so it cannot prolong a concurrently blocked account. Cookie mutations require exact configured Origin. Rotation revokes the old session and creates a fresh one in one transaction; revocation uses a conditional authoritative write that cannot be reversed by a stale touch.
