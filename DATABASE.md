...
DEV-009 defines a physically separate `noma-postgres-staging` Render PostgreSQL 18 resource in Frankfurt with external ingress disabled and encrypted application transport enforced by the staging command wrapper. The API is the single pre-deploy migration owner and invokes the existing `pnpm db:migrate:deploy` history before the new API starts. Worker never migrates. No schema, checksum, seed, production database, or production data is added by the deployment skeleton.
